...
export class ResolverPuzzle implements OnInit {
  ...
  objetos: any[] = []; // objetos de la categoría seleccionada
  caracC: number[] = [];
  caracF: number[] = [];
  ...
  //     next: (response: any) => {
  //       // Extraer el array de objetos desde response.objeto
  ...
  //   // pedir al backend crearSudoku (devuelve [caracC, caracF, tabla] o { message: [...] })
  ...
  //       // Intenta extraer desde data.body.Categoria (HttpResponse)
  ...
  //       }
  //       // Intenta extraer desde data.Categoria (respuesta directa)
  ...
  //       }
  //       // Intenta extraer array directo
  //       else if (Array.isArray(data) && data.length === 3) {
  //         arrays = data;
  //         console.log('Extraído como array directo:', arrays);
  //       }
  //       // Intenta extraer desde data.message
  ...
  //         this.tabla = Array.isArray(arrays[2]) ? arrays[2].slice(0, 9).map((v: any) => Number(v)) : Array(9).fill(0);
  //       } else {
  //         this.caracC = [];
  //         this.caracF = [];
  //         this.tabla = Array(9).fill(0);
  ...
  //       console.log('tabla asignada:', this.tabla);
  ...
  this.caracC = [];
  this.caracF = [];
  this.tabla = Array(9).fill(0);  // ← Aquí se resetea a vacío
  this.objetos = [];
  this.resultado = null;
  this.error = '';
  ...
  // pedir al backend crearSudoku (devuelve [caracC, caracF, tabla])
  this.categoriaService.sudoku(this.selectedCategoriaId).subscribe({
    next: (data: any) => {
      console.log('Respuesta crearSudoku:', data);
      ...
      if (arrays) {
        this.caracC = Array.isArray(arrays[0]) ? arrays[0].map((v: any) => Number(v)) : [];
        this.caracF = Array.isArray(arrays[1]) ? arrays[1].map((v: any) => Number(v)) : [];
        // ← NO asignamos tabla del backend, la dejamos vacía
        // this.tabla = Array.isArray(arrays[2]) ? arrays[2].slice(0, 9).map((v: any) => Number(v)) : Array(9).fill(0);
      } else {
        this.caracC = [];
        this.caracF = [];
  ...
  //   // usuarioId: si tienes sesión, puedes obtenerlo de sessionStorage, aquí se intenta leer
  //   const usuarioIdStr = sessionStorage.getItem('usuarioId') || sessionStorage.getItem('id') || '';
  //   const usuarioId = usuarioIdStr ? Number(usuarioIdStr) : null;

  //   if (!usuarioId) {
  //     this.error = 'Usuario no identificado (no hay usuarioId en sessionStorage)';
  //     return;
  //   }

  //   this.cargando = true;
  //   // llamar al servicio para verificar resolución
  ...
  // util: obtener nombre de objeto por id
  ...
  // Obtener usuarioId desde sessionStorage.user (parseando JSON)
